...
async def transform_daily(profile:models.Profile, data:dict, end_date:date=date.today()):
    ...
    for d in range((end_date - start_date).days + 1):
        ...
        dates_dict[key] = {'date': start_date + timedelta(days=d), 'calories_eaten_today': 0.0, 'fats_eaten_today':0.0, 'carbs_eaten_today':0.0, 'protein_eaten_today':0.0, 'user_inputed_weight': None, "user_activity_level":None}
    ...
    for k, v in dates_dict.items():
        act_level = v['user_activity_level'] if v['user_activity_level'] else profile.activity_level
        ...
        calories_burned = round(((weight_calc+height_calc-age_calc) + sex_calc) * act_level,0)
        day = (v["date"] - start_date).days +1

        calorie_goal = max(calories_burned - profile.lbs_per_week * 500, 1200 if profile.sex == 'female' else 1500)

        total_rmr += calories_burned
        ...
        logs.append(log)
        est_weight -= (calories_burned-calories_eaten_today)/3500
