...
async def update_actual_weight(
    *,
    deps: LoggedInDeps,
    actual_weight: bool = False,
    activity_level: bool = False,
    date: date,
    data: schemas.DailyOptions
):
    daily_data = {
    ...
    data = await api_daily.update_daily(
        deps=deps,
        current_date=date,
        daily_data=schemas.DailyOverviewInput(**daily_data),
        actual_weight=actual_weight,
        activity_level=activity_level,
    )
    context = {"request": deps["request"], "hx_request": deps["hx_request"], "user": deps["profile"], "daily": data}
